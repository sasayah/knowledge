...
1. `--publish <slug>`: 公開先URLを聞き、`git mv` で `essays/published/` へ移動、`status: published` と `published_url` を更新、コミット `essay: publish <slug>` → `git push`（失敗しても警告のみで続行）して終了。
...
7. コミット: `essay: draft <slug>` → `git push`（失敗しても警告のみで続行。リモート未設定なら黙ってスキップ）
